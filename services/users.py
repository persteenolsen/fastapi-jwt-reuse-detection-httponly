...
async def cleanup_refresh_tokens(db: Session = Depends(get_db)):
    ...
    expired = db.query(RefreshToken).filter(RefreshToken.expires_at < cutoff).all()

    tokens = set(revoked + expired)
    ...
    return {"message": "Cleanup completed", "deleted_tokens": len(tokens)}
...
async def get_tokens_and_type(refreshToken: str, db: Session):
...
def get_token_from_request(request: Request, token: str | None):
...
def get_current_username(request: Request, token: str = Depends(get_access_token)):
    username = verify_token(token, expected_type="access")
